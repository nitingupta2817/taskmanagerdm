# ============================================================
# Advanced CRM - Task Manager (with Projects + Monthly Targets)
# ============================================================
# IMPORTANT DB UPDATES YOU MUST APPLY IN SUPABASE (SQL):
#
# 1) Projects table
# ------------------------------------------------------------
...
# end$$;
#
# 2) Add project column in tasks table
# ------------------------------------------------------------
...
#
# 3) Monthly targets table (if not already created)
# ------------------------------------------------------------
...
# end$$;
#
# 4) Updated_at trigger for monthly_targets (reuse your existing set_updated_at())
# ------------------------------------------------------------
# do $$
# begin
...
def get_all_projects():
    # If table doesn't exist, Supabase will error. You must create it using the SQL above.
...
def upsert_monthly_target(project: str, month_dt: date, task: str, target_qty: int):
    ...
    # requires UNIQUE(project, month, task)
...
def update_task(task_id, task=None, assigned_to=None, status=None, remarks=None, quantity=None,
    ...
    if project is not None: update_data["project"] = project
...
# ---------------- MAIN (ROLE-BASED) ----------------
    ...
    if st.session_state.role == "Admin":
        menu = ["User Management", "Task List Management", "Add New Task", "Task Management", "To-Do", "Reports", "Monthly Targets"]
    else:
    ...
    # ---------- TASK LIST MANAGEMENT (Admin) ----------
            ...
            with col_b:
                ...
                if st.button("Delete Selected Types", key="del_task_types_btn"):
                    if not types_to_delete:
                        ...
                        if skipped: st.warning("Skipped (in use): " + ", ".join(sorted(skipped)))
                        _rerun()
        ...
        with tab2:
            try:
                ...
                st.error("Projects table not found. Create it in Supabase using the SQL in the code comments.")
                st.stop()
    ...
    # ---------- MONTHLY TARGETS (Admin) ----------
        ...
        try:
            ...
            st.error("Projects table not found. Create it in Supabase.")
            st.stop()
    ...
    # ---------- TASK MANAGEMENT (Admin) ----------
        ...
        if tasks:
            task_list = [
                f"{t['id']} | {t.get('project','')} | {t['task']} | {t['assigned_to']} | {t['status']} | {t.get('date','')} | {t.get('deadline','')} | Qty:{t.get('quantity',1)} | Done:{t.get('quantity_done',0)}"
                for t in tasks
            ]
    ...
    # ---------- REPORTS (Admin) ----------
        ...
        st.subheader("Task Reports (with Project Status)")
        ...
        project_filter = st.selectbox("Filter by Project", ["All"] + projects) if projects else "All"
        ...
        with col_f2:
            if view_mode == "Date-wise":
                ...
                if filtered_df["date"].notna().any():
                    unique_months = sorted(filtered_df[filtered_df["date"].notna()]["date"].dt.to_period("M").unique())
                else:
                    unique_months = []
        ...
        total_tasks = int(len(filtered_df))
        done_tasks = int((filtered_df["status"] == "Done").sum())
        half_tasks = int((filtered_df["status"] == "Half").sum())
        pending_tasks = int((filtered_df["status"] == "Pending").sum())

        total_units = int(filtered_df["quantity"].sum())
        done_units = int(filtered_df["quantity_done"].sum())
        remaining_units = int(filtered_df["remaining_units"].sum())

        m1, m2, m3, m4, m5, m6 = st.columns(6)
        m1.metric("Tasks", total_tasks)
        m2.metric("Done", done_tasks)
        m3.metric("Half", half_tasks)
        m4.metric("Pending", pending_tasks)
        m5.metric("Units Done", done_units)
        m6.metric("Units Pending", remaining_units)

        st.divider()

        # Project status block (task-wise by user + pending breakdown)
        st.markdown("## Project Status Breakdown")

        # 1) Task-wise done by user
        by_user_task = (
        ...
        # 3) Pending tasks list (one-by-one pending)
        ...
        # 4) Monthly target comparison (only makes sense in month-wise)
